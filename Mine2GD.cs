...
public class Mine2GD
{
    public static void Main(string[] args)
    {
        ...
        if (args.Length == 0)
            throw new ArgumentException("Please add the world file as a parameter.\nUse -h for help.");

        if (args.Contains("-h") || args.Contains("--help"))
        {
            Console.WriteLine("Usage: mine2gd.exe <file> [options] [chunk_width] [chunk_height] [chunk_length]\n");
            Console.WriteLine("Options:");
            Console.WriteLine("  -h, --help      Show help information.");
            Console.WriteLine("  -w, --write-all Write all blocks instead of only border/exposed blocks.\n");
            Console.WriteLine("You can set any of the chunk size options to 0 to not use chunks for that axis. (will probably generate big file)");
            return;
        }
        ...
        for (int x = 0; x < mine.Width; x++)
        {
            ...
            if (writeAll)
            {
                blockPos.Add(new Tuple<Vector3, Block>(new Vector3(x, y, z),
                    (mine.Blocks[index] < 51) ? (Block)mine.Blocks[index] : (Block)50));
                continue;
            }

            // check if border
            bool isAtBorder = (x == 0 || x == mine.Width - 1 ||
                               y == 0 || y == mine.Height - 1 ||
                               z == 0 || z == mine.Depth - 1);
            ...
            if (!isAtBorder)
            {
                ...
                for (int dx = -1; dx <= 1 && !nearTransparent; dx++)
                {
                    ...
                    if (nx >= 0 && nx < mine.Width &&
                        ny >= 0 && ny < mine.Height &&
                        nz >= 0 && nz < mine.Depth)
                    {
                        int neighbor = ny * (mine.Depth * mine.Width) + nz * mine.Width + nx;
        ...
        var chunkNumX = chunkSizeX != 0 ? (mine.Width + chunkSizeX - 1) / chunkSizeX : 1;
        var chunkNumY = chunkSizeY != 0 ? (mine.Height + chunkSizeY - 1) / chunkSizeY : 1;
        var chunkNumZ = chunkSizeZ != 0 ? (mine.Depth + chunkSizeZ - 1) / chunkSizeZ : 1;
        ...
        for (var chunkY = 0; chunkY < chunkNumY; chunkY++)
        {
            for (var chunkZ = 0; chunkZ < chunkNumZ; chunkZ++)
            {
                ...
                {
                    var chunkPos = blockPos.Where(p =>
                        p.Item1.X >= chunkX * chunkSizeX && p.Item1.X < (chunkX + 1) * chunkSizeX &&
                        p.Item1.Y >= chunkY * chunkSizeY && p.Item1.Y < (chunkY + 1) * chunkSizeY &&
                        p.Item1.Z >= chunkZ * chunkSizeZ && p.Item1.Z < (chunkZ + 1) * chunkSizeZ
                    ).ToArray();

                    var tscn = PlaceBlocks(chunkPos, new Vector3(chunkX, chunkY, chunkZ));

                    var file = $"{Path.GetFileName(path)}_{chunkX}_{chunkY}_{chunkZ}.tscn";
    ...
    private static string PlaceBlocks(Tuple<Vector3, Block>[] blocks, Vector3 chunkCoords)
    {
        ...
        {
            sb.AppendLine($"[ext_resource type=\"Texture2D\" path=\"res://Blocks/{((Block)id).ToString().ToLower()}.png\" id=\"Texture_{(Block)id}\"]");
        }
        
        // do the same for the material
        for (int id = 0; id < 50; id++)
        {
            sb.AppendLine($"[sub_resource type=\"StandardMaterial3D\" id=\"Material_{(Block)id}\"]");
        ...
        sb.AppendLine($"[node name=\"Chunk_{chunkCoords.X}-{chunkCoords.Y}-{chunkCoords.Z}\" type=\"Node3D\"]");

        foreach (var block in blocks)
        {
            string blockCoords = $"{block.Item1.X}-{block.Item1.Y}-{block.Item1.Z}";  

            sb.AppendLine($"[node name=\"{block.Item2}_{blockCoords}\" type=\"Node3D\" parent=\".\"]");
            sb.AppendLine($"transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, {block.Item1.X}, {block.Item1.Y}, {block.Item1.Z})");

            sb.AppendLine($"[node name=\"{block.Item2}\" type=\"MeshInstance3D\" parent=\"{block.Item2}_{blockCoords}\"]");
            sb.AppendLine($"mesh = SubResource(\"Block\")");
            
            sb.AppendLine($"material_override = SubResource(\"Material_{block.Item2}\")");
        }
